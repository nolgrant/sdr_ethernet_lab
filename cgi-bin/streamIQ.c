...
int main(int argc, char *argv[])
{
    ...
    *(radio_periph+RADIO_TUNER_CONTROL_REG_OFFSET) = 2; 
    ...
    int port       = 8080;
    int numPackets = 1; // for now, only write 10 packets
    printf("IP Address: %s\n", targetIP);
    ...
    for ( uint32_t i = 0; i < numPackets; i++){   
        ...
        while(r_cnt < nread){
            fifo_occupancy = *(fifo_periph+FIFO_RX_OCCUPANCY_REG_OFFSET);
            if(fifo_occupancy > 0){
                // Read from FIFO and add to packet
                r_val = *(fifo_periph+FIFO_RX_DATA_REG_OFFSET);
                tmpword = (uint8_t*)&r_val;
                myArray[((r_cnt)*4)+0] = tmpword[3];
                myArray[((r_cnt)*4)+1] = tmpword[2];
                myArray[((r_cnt)*4)+2] = tmpword[1];
                myArray[((r_cnt)*4)+3] = tmpword[0];
                r_cnt++;
