...
int main(int argc, char *argv[])
{
    ...
    // Allow user to specify IP and Port
    if (argc != 3)
    {
    ...
    // UDP code was sourced from Google AI (with some modifications)
    ...
    // Build UDP message
    unsigned char myArray[1028]; // 1024 bytes of samples + 4 bytes of header 
    size_t myArraySize = sizeof(myArray);
    unsigned int nread = 256; // read 256 IQ samples per packet (1024 bytes)
    ...
    printf("Stopped Packet Generation");
    return 0;
}
